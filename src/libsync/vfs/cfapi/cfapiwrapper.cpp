    ...
    const QString &syncRootPath)
{
    ...
    try {
        ...
        createSyncRootRegistryKeys(providerName, folderAlias, navigationPaneClsid, displayName, accountDisplayName, syncRootPath);
        Sleep(1000);
    } catch (...) {
        throw;
    }
...
OCC::Result<void, QString> OCC::CfApiWrapper::registerSyncRoot(const QString &path, const QString &providerName,
    const QString &providerVersion, const QString &folderAlias, const QString &navigationPaneClsid,
...
OCC::Result<OCC::Vfs::ConvertToPlaceholderResult, QString> OCC::CfApiWrapper::convertToPlaceholder(const QString &filename, time_t modtime, qint64 size, const QByteArray &fileId, const QString &replacesPath)
{
    Q_UNUSED(modtime);
    Q_UNUSED(size);

    const auto handle = handleForPath(filename);
